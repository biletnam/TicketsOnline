...
<script>
	var amount = 0; 
	var artist = '<%=new SQLServerConnection().consultar1Valor("select titulo from tbEventos where eventoPkId = "+request.getSession().getAttribute("eventId").toString())%>';
	var place = '<%=new SQLServerConnection().consultar1Valor("select b.descripcion from tbEventos a join tbEspaciosLugares b on a.claveLugar = b.clave where a.eventoPkId = "+request.getSession().getAttribute("eventId").toString())%>';
	var dateTime = '<%=new SQLServerConnection().consultar1Valor("select CONVERT(varchar, FechaHora, 102) from tbEventos where eventoPkId = "+request.getSession().getAttribute("eventId").toString())%>';
	var seats = [];
